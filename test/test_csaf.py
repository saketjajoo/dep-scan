...
from depscan.lib.csaf import (
    ...
    parse_toml,
    verify_components_present, version_helper,
)
...
def test_verify_components_present():
    ...
    assert template['document']['notes'] == [{'category': 'legal_disclaimer',
                         'text': 'Depscan reachable code only covers the '
                                 ...
                                 'mind.'}]
    assert template['product_tree'] == {'full_product_names': [{'name': 'vuln-spring',
                                          'product_id': 'vuln-spring:0.0.1-SNAPSHOT',
                                          'product_identification_helper': {'purl': 'pkg:maven/com.example/vuln-spring@0.0.1-SNAPSHOT?type=jar'}}]}

    assert new_metadata['tracking'] == {'current_release_date': '2023-11-12T06:51:08',
              'id': '',
              'initial_release_date': '2023-11-12T06:51:08',
              'status': 'draft',
              'version': '1'}
...
def test_version_helper():
    ...
    # FIXME: Something isn't right here... should not return True
    #  Returns False if the lower bound modifier is ">" and the version in
